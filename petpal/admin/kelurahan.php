                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered text-light" id="dataTable" width="100%" cellspacing="50"style="background-color: Pink;">
                                    <thead>
                                        ...
                                        <tbody>
                                            <?php 
                                              $ambildata =mysqli_query($koneksi, "SELECT * FROM kelurahan JOIN kecamatan ON kelurahan.id_kecamatan=kecamatan.id_kecamatan");
                                                 $No =1 ;
                                                while ($db= $ambildata->fetch_assoc()){
                                            ?>
                                               <tr>
                                                   <td><?php echo $No?></td>
                                                   <td><?php echo $db['id_kelurahan'];?></td>
                                                   <td><?php echo $db['nama_kelurahan'];?></td>
                                                   <td><?php echo $db['nama_kecamatan'];?></td>
                                                    
                                                    <td>  
                                                        <a href ="hapus_kelurahan.php?id_kelurahan=<?php echo $db['id_kelurahan']?>" onclick ="return confirm ('Apakah anda yakin ingin menghapus data?')"name="hapus" class="btn btn-purple"> Hapus </a>   
